...
orderjs(function(){
	function doAction() {
		...
		if (rlx.mti.validate(mainForm)) {
			mainForm.action = "${base}portal/blackWhiteUpdate.do";
...
<div id="mainContent" class="rlx_mti_info">
    <form id="mainForm" name="mainForm" method="post" onsubmit="return false;">
        ...
        <input type="hidden" name="bw" value="${dto.bw}" />
        <div class="group">
            <table border="0" class="grid_fields grid_fields_cols4">
                <tr>
                    <th class="label">MAC</th>
                    <td class="field_ctn">
                        <input class="field" name="mac" dataType="Require" value="${dto.mac}" msg="必填项" />
                    </td>                    
                    <th class="label">备注</th>
                    <td class="field_ctn">
                        <input class="field" name="reason" value="${dto.reason}" />
                    </td>
                </tr>    
            </table>
        </div>
