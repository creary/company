...
orderjs(function(){
	...
	var dropdownItemActions = {
		"rename" : function(){
			...
			var url = "${base}cpe/richHostReadyEdit.do?id=" + row.getValue("id");
			rl.page.open(url, "cpe.readyEditName", rl.gui.paneDlgOpt_l);
		},
		...
		"debug" : function(){
			...
			var row = getRow(dropdownItemRowIndex);
			var url = "${base}cpe/readySetScheduleReboot.do?batchMode=0&id=" + row.getValue("id");
	...
	window.openDeviceGroupEdit = function (){
		var url = '${base}group/readyTree.do?flag=2';
	...
	rlx.mti.createTreeListPage({
		...
		toolbar : {
			items : [		
				...
				{
					text : "分布图",
					icon : "${imagePath}topo.gif", 
					action : function(){
						var url = "${base}cpe/readyGetDeviceLayout.do";
						    rl.page.open(url, "cpe.readyBatchSetting", rl.gui.winDlgOpt);
					}
				},
				"search",
		...
		grid :{
			...
			"mode","description","trace","detect","kickStaRssiLow","assocReqRssiThres","authRssiThres","wirelessTxPower","maxStaNum","code","location",
			"usedText","ssidName","portalName","policyName"],
			dataPrimaryKey : "id",
			...
			columns : [
				...
				{caption: "名称", width: 150, name: "name", convert : genNameCol},
				{caption: "序列号(MAC)", name: "serialNumber"},
				{caption: "编号", width: 60, name: "code"},
				{caption: "位置", width: 120, name: "location"},
				{caption: "是否开通", width: 60, name: "usedText"},
				...
				{caption: "功率(%)", name: "wirelessTxPower", visible : false, colEntityTag : "wireless"},
				...
				{caption: "SSID策略", name: "ssidName", visible : false, colEntityTag : "operation"},							
				{caption: "Portal策略", name: "portalName", visible : false, colEntityTag : "operation"},
                {caption: "认证策略", name: "policyName", visible : false, colEntityTag : "operation"},          								
...
<div id="primaryColDropdown" class="soof_drop_menu">
	<a class="item" href="javascript:void(0);" data-key="rename">修改设备属性</a>
    <div class="sep"></div>
...
<div style="display:none;">
    <div id="deviceColumnsSeletor" class="rl_btn jquery_idtabs radio_btn">
        <ul class="tabbar">
            ...
            <li><a title="显示设备的无线信息" href="#deviceColumns_wireless">无线信息</a></li>
            <li><a title="显示设备的运营信息" href="#deviceColumns_operation">运营信息</a></li>
        </ul>
    </div>
</div>
